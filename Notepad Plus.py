...
from quichem.compilers.plain import PlainAsciiCompiler
from quichem.parser import parse, make_parser
...
class ChemicalMapping():
    def __init__(self):
        ...
        self.jumpDistanceLimit = 100000
        self.jumpDistanceMax = 1000
        self.jumpDistanceMin = 100

        self.shootMax = 1000
        self.shootMin = 100
        self.shootDistance = 5000
        self.depthTimes = 2
    ...
    def summonObjects(self):
        for equation in self.equations:
            ...
            equation = _public_api.balance_chemical_equation(
                equation, opt, printer=_public_printer.PRINTER_TEXT, unknown_header="X")

            for reactant in reactants:
                if not reactant in self.objects.keys():
                    self.objects[reactant] = {
                        "to": {}, "from": {}, "isPrecipitation": False, "isGas": False}
    ...
    def toXmind(self, objName, tdeepth=10, filename="result.xmind"):
        ...
        root_topic.setPosition(0, 0)

        rs = {}
        usedPos = []

        def create(text, t, txt, equation):
            if txt in rs.keys():
                rb = rs[txt].getTitle()
                if rb != equation:
                    rs[txt].setTitle(rs[txt].getTitle()+"\n"+equation)
            else:
                rs[txt] = sheet.createRelationship(
                    summoned[text].getID(), summoned[t].getID(), equation)

        def createOBJ(lastPos, lastAngle, text, deepth):
            if deepth == 0:
                return
            if not text in summoned.keys():
                summoned[text] = root_topic.addSubTopic(
                    topics_type=TOPIC_DETACHED)
                summoned[text].setTitle(text)
                t = 0
                summoned[text].setPosition(random.choice([-1, 1]) * random.randint(self.jumpDistanceMin, self.jumpDistanceMax),
                                           random.choice([-1, 1]) * random.randint(self.jumpDistanceMin, self.jumpDistanceMax))
                if self.formType == "random jump" and usedPos:
                    pos1 = summoned[text].getPosition()
                    pos = usedPos[0]
                    while pos != usedPos[-1]:
                        for pos in usedPos:
                            if (pos1[0]-pos[0])**2+(pos1[1]-pos[1])**2 < self.jumpDistanceLimit:
                                summoned[text].setPosition(pos1[0] + random.choice([-1, 1]) * random.randint(self.jumpDistanceMin, self.jumpDistanceMax),
                                                           pos1[1] + random.choice([-1, 1]) * random.randint(self.jumpDistanceMin, self.jumpDistanceMax))
                                t += 1
                                pos1 = summoned[text].getPosition()
                                break
                elif self.formType == "radio":
                    dis = (random.randint(self.shootMin, self.shootMax) if self.shootDistance <
                           0 else self.shootDistance) / ((tdeepth - deepth)*self.depthTimes)
                    angle = lastAngle
                    if not summoned[text].getPosition:
                        summoned[text].setPosition(
                            lastPos[0] + math.sin(angle) * dis, lastPos[1] + math.cos(angle) * dis)
                usedPos.append(summoned[text].getPosition())
            posnow = summoned[text].getPosition()
            if self.objects[text]["to"]:
                angle = 180
                per = 180 / len(self.objects[text]["to"])
                for t, equation in self.objects[text]["to"].items():
                    if t in summoned.keys():
                        txt = text + " " + t
                        if self.formType == "radio":
                            dis = (random.randint(self.shootMin, self.shootMax) if self.shootDistance <
                                   0 else self.shootDistance) / ((tdeepth - deepth)*2)
                            summoned[t].setPosition(
                                posnow[0] + math.sin(angle) * dis, posnow[1] + math.cos(angle) * dis)
                        create(text, t, txt, equation)
                    else:
                        createOBJ(posnow, angle, t, deepth - 1)
                    angle -= per
            if self.objects[text]["from"]:
                angle = 180
                per = 180 / len(self.objects[text]["from"])
                for f, equation in self.objects[text]["from"].items():
                    if f in summoned.keys():
                        ...
                        if self.formType == "radio":
                            dis = (random.randint(self.shootMin, self.shootMax) if self.shootDistance <
                                   0 else self.shootDistance) / ((tdeepth - deepth)*2)
                            summoned[f].setPosition(
                                posnow[0] + math.sin(angle) * dis, posnow[1] + math.cos(angle) * dis)
                        create(text, f, txt, equation)
                    else:
                        createOBJ(posnow, angle, f, deepth - 1)
                    angle -= per

        createOBJ([0, 0], 0, objName, tdeepth)
...
class E():
    def __init__(self):
        ...
        self.fonti_title = ImageFont.truetype(
            "simsun.ttc", 100, encoding="utf-8")
        ...
        self.mainEdit.bind("<Control-q>", self.quichemUI)
        self.mainEdit.bind("<Control-Q>", self.quityper)
    ...
    def quichemUI(self, e):
        os.system(".\\quichem-pyside_2014-09-04_win32\\quichem-pyside.exe")

    def quityper(self, ev):
        e = tk.Entry(self.mainEdit, font=self.font)

        def enterQ(ev):
            self.mainEdit.insert("end", _public_api.balance_chemical_equation(compiler.compile(parse(e.get(), parser)).replace(
                ' ', '').replace("->", '='), opt, printer=_public_printer.PRINTER_TEXT, unknown_header="X") + '\n')
            e.destroy()
            self.quityper(None)
        e.bind("<Return>", enterQ)
        e.focus()
        self.mainEdit.window_create("end", window=e)

    def read(self, filename=None):
        try:
            with open(filename if filename else (datetime.now().strftime('%Y-%m-%d')+".txt"), "r", encoding="utf-8") as f:
    ...
    def enter(self, e):
        ...
        if cmd == "c":
            ...
            for e in es:
                if e and e[0] != "-":
                    self.chemicalMapping.equations.append(e)
            ...
            cs = cmd.split(" ")
            if cs[1] == "radio":
                self.chemicalMapping.jumpDistanceLimit = cs[2]
                self.chemicalMapping.jumpDistanceMax = cs[3]
                self.chemicalMapping.jumpDistanceMin = cs[4]
            elif cs[1] == "randomJump":
                self.chemicalMapping.shootDistance = cs[2]
                self.chemicalMapping.shootMax = cs[3]
                self.chemicalMapping.shootMin = cs[4]
                self.chemicalMapping.depthTimes = cs[5]
    ...
    def shift_enter(self, e):
        if self.editing:
            ...
            chooser["value"] = self.get_sentences(
                self.mainEdit.get("insert linestart", "insert lineend"))
            ...
            chooser.bind("<Shift-Return>",
                         lambda e: self.complete_choosing(chooser, False))
        ...
        if "dict_result" in result.keys():
            if "exchange" in result["dict_result"]["simple_means"]:
                [self.exchanges.extend(
                    exchanges) for exchanges in result["dict_result"]["simple_means"]["exchange"].values()]
                    ...
                    for entry in menu["entry"]:
                        ...
                        for value in entry["value"]:
                            for mean_type in value["mean_type"]:
                                if mean_type["info_type"] != "example":
                                    continue
                                for example in mean_type["example"]:
                                    result_a.append("[{}][{}]{}~{}".format(
                                        value["posp"][0]["label"], value["tran"], example["ex"], example["tran"]))
            else:
    ...
    def saveFile(self):
        with open(datetime.now().strftime('%Y-%m-%d')+".txt", "w", encoding="utf-8") as f:
    ...
    def fontSeter(self, font, font_title):
        ...
        if font_title != self.font_i_title:
            ...
            self.fonti_title = ImageFont.truetype(
                self.font_i_title, 100, encoding="utf-8")
    ...
    def save(self, filename="result.png", mixed=False):
        ...
        if mixed:
            for title, text in buffer.items():
                line += 40
                draw.text(((2479 - self.fonti_title.getsize(title)
                            [0]) / 2, line), title, font=self.fonti_title, fill=(0, 0, 0))
                line += 100
                ...
                for word in text.keys():
                    draw.text((90, line), word,
                              font=self.fonti, fill=(0, 0, 0))
                    line += 50
                result_buffer = []
